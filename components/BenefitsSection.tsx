  ...
  useEffect(() => {
    const loadBenefits = async () => {
      try {
        ...
        if (response.ok) {
          const data: BenefitsData = await response.json()
          // Фильтруем только активные преимущества и сортируем по порядку
          const activeBenefits = data.benefits?.filter(benefit => benefit.active) || []
          activeBenefits.sort((a, b) => (a.order || 0) - (b.order || 0))
          setBenefits(activeBenefits)
        } else {
          console.error('Failed to load benefits, status:', response.status)
        }
      } catch (error) {
                ...
                <div className="relative h-40 md:h-48 overflow-hidden">
                  ...
                  <div className="absolute bottom-3 md:bottom-4 left-3 md:left-4">
                    <div className="w-10 h-10 md:w-12 md:h-12 bg-white/90 backdrop-blur-sm rounded-xl flex items-center justify-center shadow-lg">
                      <IconComponent className="w-5 h-5 md:w-6 md:h-6 text-coffee-600" />
                    </div>
                  </div>
                </div>
